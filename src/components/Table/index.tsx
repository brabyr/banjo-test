...
export default function CustomPaginationActionsTable({ handleOpen, tableData } : TableProps) {
  return (
        <Card sx={{ borderRadius: '16px', maxWidth: 1110, margin: 'auto' }}>
                    ...
                    <TableBody>
                        {tableData.map((item: any) => (
                            <TableRow key={item.teamMember}>
                                <TableCell align="left" sx={{ fontSize: '12px', fontWeight: 600, lineHeight: '20px', width: '331px'}}>{item.name}</TableCell>
                                <TableCell align="left" sx={{ fontSize: '14px', fontWeight: 400, lineHeight: '20px', color: '#425466', width: '166px'}}><Box sx={{ display: 'flex', alignItems: 'center'}}><Box sx={{ width: '10px', height: '10px', marginRight: '10px', background: `${item.priority === 'Low' ? '#F16063' : item.priority === 'Medium' ? '#F7936F' : '#66CB9F'}`, borderRadius: '50%'}}></Box>{item.priority}</Box></TableCell>
                                <TableCell align="left" sx={{ fontSize: '14px', fontWeight: 400, lineHeight: '20px', color: '#425466', width: '155px'}}>{item.id}</TableCell>
                                <TableCell align="left" sx={{ fontSize: '12px', fontWeight: 600, lineHeight: '20px', color: '#425466', width: '209px'}}>{item.team_name}</TableCell>
                                <TableCell align="left" sx={{ fontSize: '14px', fontWeight: 400, lineHeight: '20px', color: '#425466', width: '181px'}}>{item.date}</TableCell>
